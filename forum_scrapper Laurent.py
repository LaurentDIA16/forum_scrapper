...
""" 1/ FRIGO Fonctionnel pour la correction des bugs pages et posts
/**
    URL = "https://community.o2.co.uk/t5/Discussions-Feedback/bd-p/4"
    domain = urlparse(URL).netloc #community.o2.co.uk

    all_threads = [] #récupère les titres et url des articles de chaque page (13 x 931 pages)
    ...
    for all_pages in range(1,3):
        ...
        for thread_title in results:
            ...
            all_threads.append((title,url)) #titre et url des articles qui va s'accumuler au fil des pages
...
""" 2 / FRIGO Fonctionnel ajout du nombre de vue pour chaque article
...
/**
    ...
    for all_pages in range(1,3):
        ...
        for thread_title in results:
            ...
            #Créer un fichier txt poury inscrire les titres, url, views 
            chemin = r"C:\Users\devia.e16\Documents\GitHub\forum_scrapper\liste titre d'article.txt"
            with open(chemin, 'w') as f:
                f.write(f"{threads}")
...
with open("Liste d'articles.csv","w",newline="") as file:
            file.writerow(f"{all_threads}")
        
        #Créer un fichier csv pour y inscrire les titres, url, views 
        headerList = ["Titres","URL","Nb de vues"]
        with open("Liste d'articles.csv","w",newline="") as file:
            dw = csv.DictWriter(file, delimiter=',',fieldnames=headerList)
            dw.writeheader()
            #dw.writerows(all_threads)
"""